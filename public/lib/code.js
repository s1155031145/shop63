...
var json_computer = {
	...
	"category_id": "02",
    "product_list": [
        {"name": "Monster M7 7in Tablet 16GB Bonus 4ft microUSB Cable", "id": "02,01", "img":"000008.jpg", "price":"59.99", "description":"Monster has engineered the finest in mobile computing. The Monster M7 Tablet has the sleek look and quality craftsmanship that you have come to expect from Monster. Now, this is being brought to you at an incredible price. You can email, surf the Internet, read eBooks, view photos, play games, listen to music and watch HD video. With Google Play you can access thousands of applications with plenty of games, social media, and productivity tools to meet your needs. The Monster® M7 Tablet looks as cool as it works. Enjoy doing more with the Monster M7 Tablet."},
        {"name": "Apple iPad Air 2 Choose(16GB,64GB,128GB) Wi-Fi Bonus Justin Case", "id": "02,02", "img":"000009.jpg", "price":"499.99", "description":"Apple iPad Air 2 Choose(16GB,64GB,128GB) Wi-Fi Bonus Justin Case Apple iPad Air 2 Choose(16GB,64GB,128GB) Wi-Fi Bonus Justin Case"},
    ]
};
...
var json_food = {
	...
	"category_id": "03",
    "product_list": [
        {"name": "Ferrero Rocher: Chocolates Fine Hazelnut, 5.3 oz", "id": "03,01", "img":"000010.jpg", "price":"3.98", "description":"Ferrero Rocher: Chocolates Fine Hazelnut is a delicious treat for almost any occasion. With fine hazelnut, this chocolate and confection candy will delight friends and family. The hazelnut chocolate will also help satisfy a chocolate lover's craving. Since the 1940s, Ferrero has delighted consumers with premium, one-of-a-kind, quality confections. Share a 5.3-oz box of Ferrero Rocher: Chocolates Fine Hazelnut today."},
    ]
};
...
var json_shop_list = {
	"total":"62.88",
	"shop_list": [
		{"name":"Big Hero 6 (Blu-ray + DVD + Digital HD) (Widescreen)", "id":"01,01", "price":"18.96", "quantity":"1"},
		{"name":"Marvel: Guardians Of The Galaxy (Blu-ray) (Widescreen)", "id":"01,03", "price":"21.96", "quantity":"2"}
	]
};
...
function toProductList(id){
	...
	sessionStorage.category = id;
	sessionStorage.product = '00';
}
...
function toProductDetail(category_id, id){
	...
	sessionStorage.category = category_id;
	sessionStorage.product = id;
}
...
function addToCart(name,id,price){
	...
	for (var i = 0; i < json_shop_list.shop_list.length; i++){
		console.log(json_shop_list.shop_list[i].id + ", " + id);
		if (json_shop_list.shop_list[i].id == id){
			json_shop_list.shop_list[i].quantity = +json_shop_list.shop_list[i].quantity + 1;
			added = 1;
		}
	}
	if (added == 0){
		var temp_shop_list = {"name":name, "id":id, "price":price, "quantity":"1"};
		json_shop_list.shop_list.push(temp_shop_list);
	}
	var total = 0;
	for (var i = 0; i < json_shop_list.shop_list.length; i++){	
